...
def ErrorRate_test1(ps):
    ...
    for sig in range(ps.n//2):
        s = sqrt((1.5 * ps.n - sig - 1) * ps.sigma1**2 * (2 * ps.sigma2**2 + (ps.q/2**ps.t)**2 * div_t) + ps.sigma2**2)
        L.append(s)
    for sig in range(ps.n//2):
        s = sqrt((1.5 * ps.n) * ps.sigma1**2 * (2 * ps.sigma2**2 + (ps.q/2**ps.t)**2 * div_t) + ps.sigma2**2)
        L.append(s)
...
def ErrorRate_test2(ps):
    ...
    L2 = []
    for sig in range(ps.n//2):
        s = (1.5 * ps.n - sig - 1) * ps.sigma1**2 * (2 * ps.sigma2**2 + (ps.q/2**ps.t)**2 * div_t) + ps.sigma2**2
        L1.append(2 * s)
        L2.append(4 * s**2)
    for sig in range(ps.n//2):
        s = (1.5 * ps.n) * ps.sigma1**2 * (2 * ps.sigma2**2 + (ps.q/2**ps.t)**2 * div_t) + ps.sigma2**2
        L1.append(2 * s)
        L2.append(4 * s**2)
    for i in range(0,ps.n,8):
...
def ErrorRate_test3(ps):
    ...
    if dis > 0:
        # pr = chi2.logsf((dis/ s)**2, 8.) / log(2)
        pr = gamma.logsf((dis/ s)**2, 4, scale=2) / log(2) + log(ps.n/8,2)

        print("err of test3(最大项数):")
        print("    = 2^%.2f"% pr," (dis/s)^2 = %.2f"%(dis/ s)**2," div_t = %.2f"%div_t," dis = %.2f"%dis," s = %.2f"%s)
    else:
        print("error! dis <= 0")

# 最小项数
def ErrorRate_test4(ps):
    # variance of (q/2^t)*epsilon = (2^t/q)*u - round(2^t/q*u)
    # u is uniform in (-(q-1)/2,(q-1)/2), round(x) = floor(x + 1/2)
    div_t = 0
    avg_t = 0
    ...
    if dis > 0:
        ...
        print("err of test4(最小项数):")
        print("    = 2^%.2f"% pr," (dis/s)^2 = %.2f"%(dis/ s)**2," div_t = %.2f"%div_t," dis = %.2f"%dis," s = %.2f"%s)
    else:
        print("error! dis <= 0")

# 项数平均
def ErrorRate_test5(ps):
    # variance of (q/2^t)*epsilon = (2^t/q)*u - round(2^t/q*u)
    # u is uniform in (-(q-1)/2,(q-1)/2), round(x) = floor(x + 1/2)
    div_t = 0
    avg_t = 0
